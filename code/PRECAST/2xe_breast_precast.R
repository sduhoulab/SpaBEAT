...
project_name <- "xe_breast"
datasets <- c('Rep1_outs', 'Rep2_outs')
sample_to_sheet <- c('Xenium R1 Fig1-5 (supervised)', 'Xenium R2 Fig1-5 (supervised)')
names(sample_to_sheet) <- datasets
n_domains <- 20
celltype_path <- file.path(file_fold, "Cell_Barcode_Type_Matrices.xlsx")
...
for (j in 1:length(datasets)) {
    dataset <- datasets[j]
    file_path <- file.path(file_fold, dataset, 'cell_feature_matrix.h5')
    cat("  Loading slice", dataset, "from", file_path, "\n")
    rawdata <- Read10X_h5(file_path)
    if (is.list(rawdata)) {
        if ("Gene Expression" %in% names(rawdata)) {
            counts <- rawdata[["Gene Expression"]]
        } else {
            counts <- rawdata[[1]]
        }
    } else {
    ...
    control_genes <- grepl("^(BLANK_|NegControl_|Control_|antisense_)", rownames(counts))
    counts <- counts[!control_genes, ]
    
    spatial_path <- file.path(file_fold, dataset, 'cells.parquet')
    sp_df <- as.data.frame(arrow::read_parquet(spatial_path))
    rownames(sp_df) <- sp_df$cell_id
    
    df_meta <- read_excel(celltype_path, sheet = sample_to_sheet[dataset])
    gt_map <- setNames(df_meta$Cluster, as.character(df_meta$Barcode))
    
    all_cells <- colnames(counts)
    meta_data <- data.frame(row.names = all_cells)
    
    meta_data$row <- sp_df[all_cells, 'y_centroid']
    meta_data$col <- sp_df[all_cells, 'x_centroid']
...
PRECASTObj <- AddAdjList(PRECASTObj, platform = "Xenium") 
...
final_h5ad_path <- paste0(out_dir, "xe_breast_seuInt_with_all_spatial.h5ad")
