...
datasets <- c('Slice_5', 'Slice_6', 'Slice_7', 'Slice_8', 'Slice_9')
...
print("Loading merfish1 h5ad data...")
...
for (i in 1:length(datasets)) {
    ...
    set.seed(2024)
    keep_cells <- sample(colnames(sce), size = round(0.5 * ncol(sce)))  # 0.5=50%
    sce <- sce[, keep_cells]
    cnts <- cnts[, keep_cells]
    meta_data <- meta_data[keep_cells, ]
    sp_coords <- sp_coords[keep_cells, ]
    cat("→ 下采样完成，剩余细胞：", length(keep_cells), "\n")
...
PRECASTObj <- CreatePRECASTObject(seuList, project = "merfish1", gene.number = 2000, 
...
saveRDS(PRECASTObj, file =  paste0(out_dir, "embryo_PRECASTObj.rds"))
...
print(seuInt)
saveRDS(seuInt, file =  paste0(out_dir, "embryo_seuInt.rds"))
...
write_json(benchmark_results, paste0(out_dir, "embryo_precast_benchmark.json"), pretty = TRUE, auto_unbox = TRUE)
...
PRECASTObj <- readRDS(paste0(out_dir, "embryo_PRECASTObj.rds"))
seuInt <- readRDS(paste0(out_dir, "embryo_seuInt.rds"))
raw_data_root <- "/data/ZhaoMH/ST0507/RAW_SLICE/stereo_embryo/Embryo_converted/" 
sample_ids <- c('Slice_5', 'Slice_6', 'Slice_7', 'Slice_8', 'Slice_9') 
raw_obj_list <- list()

print("Loading merfish1 data directly from H5ADs...")
raw_obj_list <- list()
...
zellkonverter::writeH5AD(sce_int, paste0(out_dir, "embryo_seuInt_with_all_spatial.h5ad"))
...
file_path = '/data/ZhaoMH/ST0507/PRECAST-main/results/embryo_seuInt_with_all_spatial.h5ad'
