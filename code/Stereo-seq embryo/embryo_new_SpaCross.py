...
save_path = Path('/data_hou/BE/SpaCross-main/Results')
...
for idxxx, section_id in enumerate(slices_list):
    ...
    keep_idx = np.random.choice(
        adata.n_obs,
        size=int(adata.n_obs * 0.5),
        replace=False
    )
    keep_idx = np.sort(keep_idx)
    adata = adata[keep_idx].copy()
    print(f"   [Downsample] Retained {adata.n_obs} cells for {section_id}")
...
Batch_list = TOOLS.align_spots(Batch_list, method='icp', data_type="Visium", plot=False)
