...
data_path = "/data_hou/ZhaoMH/data/stereo_embryo/Embryo_converted/" 
data_name_list = ['Slice_5', 'Slice_6', 'Slice_7', 'Slice_8', 'Slice_9']

save_path = "/data_hou/ZhaoMH/new_model/DeepST/results" 
...
for i in range(len(data_name_list)):
    ...
    print(f"Cell count before downsampling: {adata.n_obs}")
    sc.pp.subsample(adata, fraction=0.5)
    print(f"Cell count after downsampling: {adata.n_obs}")
...
del graph_list
del multiple_graph
del augement_data_list

gc.collect()
torch.cuda.empty_cache()

print("Memory after cleanup:",
      get_memory_usage()/1024, "GB")
...
multiple_adata.write(os.path.join(save_path, "multiple_adata_embryo.h5ad"))

with open(os.path.join(save_path, "deepst_benchmark_embryo.json"), "w") as f:
