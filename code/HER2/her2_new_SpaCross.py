...
save_path = Path('/root/ST0507/new_model/SpaCross-main/Results')
...
yaml_dir = Path('/root/ST0507/new_model/SpaCross-main/Config')
...
for idxxx, section_id in enumerate(slices_list):
    ...
    # Support both 'A1' and 'A/A1' format, extract slice name at the end
    batch_name = str(section_id).split('/')[-1]
    
    # Dynamically construct paths for h5ad and metadata.csv
    file_path = input_dir / f"{batch_name}.h5ad"
    metadata_file = input_dir / 'metadata.csv'

    # 1. Directly load converted H5AD file (replaces original read_visium)
    ...
    # ==========================================
    # 2. Dynamic loading logic for Ground Truth labels
    # ==========================================
    # For backward compatibility: copy lowercase ground_truth to 'Ground Truth'
    ...
    # Load labels from external file if labels are still missing
    if 'Ground Truth' not in adata.obs.columns:
        if metadata_file.exists():
            ...
            # Check all potential label column names (compatible with DLPFC, Her2, MERFISH and other datasets)
            possible_cols = ['ground_truth', 'layer_guess', 'celltype', 'Ground Truth', 'label', 'Cell_class']
            for col in possible_cols:
                if col in df_meta.columns:
                    # Safe mapping via barcode to avoid cell order mismatch
    ...
    # 3. Filter out NA background spots to ensure clean cell data
...
Batch_list = TOOLS.align_spots(Batch_list, method='icp', plot=False)
