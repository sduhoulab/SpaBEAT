...
import sys
sys.path.append('/root/BE/SPIRAL')
...
input_dir="/root/ST0507/data/Her2_tumor_converted/A/"
results_dir="/root/ST0507/new_model/Spiral/results/"
datasets=np.array(['A1', 'A2', 'A3', 'A4', 'A5', 'A6'])
SEP=','
net_cate='_KNN_'
rad=150
knn=8
...
with open(os.path.join(results_dir, "spiral_benchmark_her2_A.json"), "w") as f:
...
if len(coord) == ann.n_obs:
    ...
    print(f"✅ Success! Loaded {len(coord)} coordinates into AnnData object.")
else:
    raise ValueError(f"❌ Length mismatch! Coordinate table contains {len(coord)} rows, but AnnData has {ann.n_obs} cells. Please check input data.")
    
# ann.obsm['spatial'] = coord.loc[ann.obs_names, :].values
ann.obs["new_batch"] = ann.obs["batch"].astype(str)
ann.write('/root/ST0507/new_model/Spiral/results/spiral_her2_A.h5ad')
