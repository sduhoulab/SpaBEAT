...
for name, adata in name_to_adata.items():
    ...
    adata.obs_names = [name + '-' + x for x in adata.obs_names]
...
for i in np.arange(len(samples)):
    ...
    mat.to_csv("/data_hou/ST_data_new/model-zn/spiral/data/spatch_ov/"+samples[i]+"_mat.csv")
    meta.to_csv("/data_hou/ST_data_new/model-zn/spiral/data/spatch_ov/"+samples[i]+"_meta.csv")
    coord.to_csv("/data_hou/ST_data_new/model-zn/spiral/data/spatch_ov/"+samples[i]+"_coord.csv")
