...
input_dir="../data/DLPFC/processed/allsample/"
results_dir="../results/"
datasets=np.array([151673,151669,151507])
SEP=','
net_cate='_KNN_'
rad=150
knn=6
...
for i in range(len(datasets)):
    feat_file.append(input_dir + f"{datasets[i]}_features.txt")
    edge_file.append(input_dir + f"{datasets[i]}_edge_KNN_{knn}.csv")
    meta_file.append(input_dir + f"{datasets[i]}_label.txt")
    coord_file.append(input_dir + f"{datasets[i]}_positions.txt")
...
ann.obs["batch"] = ann.obs["batch"].astype(str)
new_batch_1 = ann.obs["batch"].isin(['151673'])
new_batch_2 = ann.obs["batch"].isin(['151669'])
new_batch_3 = ann.obs["batch"].isin(['151507'])
ann.obs["sample_name"] = list(sum(new_batch_1)*['Sample 1'])+list(sum(new_batch_2)*['Sample 2'])+list(sum(new_batch_3)*['Sample 3'])
ann.obs["new_batch"] = ann.obs["sample_name"].astype('category')

ann.write('../results/spiral_Sample_DLPFC_all.h5ad')
...
with open(os.path.join(results_dir, "spiral_benchmark_all.json"), "w") as f:
    json.dump(benchmark_results, f, indent=2)

# =============== Print Benchmark Results ===============
print("\n" + "="*50)
print("SPIRAL Training Benchmark Results")
print("="*50)
...
print(f"Final Cells: {SPII.feat.shape[0]:,}")
print(f"Total Genes: {N:,}")
...
print(f"\nBenchmark results saved to: {results_dir}spiral_benchmark_all.json")
