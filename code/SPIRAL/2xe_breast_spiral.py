...
import sys
sys.path.append('/data_hou/BE/SPIRAL/')
...
input_dir="/data_hou/ST_data_new/model-zn/spiral/data/xe_breast/"
results_dir="/data_hou/ST_data_new/model-zn/spiral/results/"
datasets=np.array(['Rep1_outs', 'Rep2_outs'])
SEP=','
net_cate='_KNN_'
knn=12
...
coord_file=[]
for i in datasets:
    feat_file.append(input_dir + f"{i}_mat.csv")
    edge_file.append(input_dir + f"{i}_edge_KNN_12.csv")
    meta_file.append(input_dir + f"{i}_meta.csv")
    coord_file.append(input_dir + f"{i}_coord.csv")
...
with open(os.path.join(results_dir, "xe_breast_spiral_benchmark1.json"), "w") as f:
...
ann.write(os.path.join(results_dir, "xe_breast_spiral_adata1.h5ad"))
