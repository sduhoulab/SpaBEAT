...
data_path = "/data/ZhaoMH/ST0507/RAW_SLICE/merfish_mouse1/data/MERFISH_converted/" 
data_name_list = ['Slice_7', 'Slice_8', 'Slice_9', 'Slice_10', 'Slice_11']
...
for i in range(len(data_name_list)):
    ...
    adata = deepen._get_augment(
        ...
        #spatial_type="BallTree", # snippet： "BallTree"
    ...
    if 'ground_truth' not in adata.obs:
        ...
        if os.path.exists(metadata_file):
            print(f"[提示] 尝试从外部 metadata.csv 读取...")
...
multiple_adata.write(os.path.join(save_path, "multiple_adata_merfish.h5ad"))

with open(os.path.join(save_path, "deepst_benchmark_merfish.json"), "w") as f:
    json.dump(benchmark_results, f, indent=2)
    
print("\n🎉 MERFISH 数据集 DeepST 整合分析完毕！结果已安全保存。")
