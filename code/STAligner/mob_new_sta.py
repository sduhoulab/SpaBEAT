...
for dataset in datasets:
    adata = sc.read_h5ad(os.path.join('../../RAW_SLICE/mob', dataset + '.h5ad'))
...
adata_concat.X = np.nan_to_num(adata_concat.X, nan=0.0, posinf=0.0, neginf=0.0)
sc.pp.filter_genes(adata_concat, min_counts=1)  # 去掉全零基因
sc.pp.filter_genes_dispersion(adata_concat)     # 也可以挑高变基因避免方差=0
...
with open("../results/staligner_benchmark_mob3.json", "w") as f:
